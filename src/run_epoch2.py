...
def softTeaching_every_batch(s_model: SSD300_3Way,
    ...
    for (batch_idx, (wL_image_vis, wL_image_lwir, sL_image_vis, sL_image_lwir, L_boxes, L_labels, L_is_anno)), \
        ...
        with torch.no_grad():
            ...
            # inference
            # Forward prop.
            fusion_teacher_locs, fusion_teacher_scores, fusion_teacher_features = t_model(w_image_vis, w_image_lwir)  # (N, 8732, 4), (N, 8732, n_classes)
            # fusion_teacher_locs, fusion_teacher_scores, _, _, _, _, fusion_teacher_features, _, _ = t_model(w_image_vis, w_image_lwir)
        ...
        # calc GAP weight
        GAP_weight, contrastive_loss = calc_weight_by_GAPVector_distance(fusion_teacher_features, L_boxes, fusion_pseudo_boxes, len(L_boxes), config.test.input_size)

        fusion_student_locs, fusion_student_scores, vis_student_locs, vis_student_scores, lwir_student_locs, lwir_student_scores, _, _, _ = s_model(s_image_vis, s_image_lwir)  # (N, 8732, 4), (N, 8732, n_classes)
        ...
        un_boxes = [box.to(device) for box in fusion_pseudo_boxes]
        un_labels = [label.to(device) for label in fusion_pseudo_labels]
        ...
        fusion_un_loss, cls_loss, loc_loss, n_positives = criterion(fusion_un_locs, fusion_un_scores, un_boxes, un_labels)
        vis_un_loss, cls_loss, loc_loss, n_positives = criterion(vis_un_locs, vis_un_scores, un_boxes, un_labels)
        lwir_un_loss, cls_loss, loc_loss, n_positives = criterion(lwir_un_locs, lwir_un_scores, un_boxes, un_labels)

        loss = fusion_sup_loss + GAP_weight * (fusion_un_loss + vis_un_loss + lwir_un_loss) + contrastive_loss
