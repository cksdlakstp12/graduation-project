...
class KAISTPedWSBatch(KAISTPedWSEpoch):
    ...
    def pull_item(self, index):
        ...
        if self.data == "KAIST":
            set_id, vid_id, img_id = frame_id[-1]
            if self.args.rebuttal == "rgb":
                vis = Image.open( self._imgpath % ( *frame_id[:-1], set_id, vid_id, 'visible', img_id ))
                lwir = Image.open( self._imgpath % ( *frame_id[:-1], set_id, vid_id, 'visible', img_id )).convert('L')
            elif self.args.rebuttal == "ir":
                vis = Image.open( self._imgpath % ( *frame_id[:-1], set_id, vid_id, 'lwir', img_id ) ).convert("RGB")
                lwir = Image.open( self._imgpath % ( *frame_id[:-1], set_id, vid_id, 'lwir', img_id ) ).convert('L')
            else:
                vis = Image.open( self._imgpath % ( *frame_id[:-1], set_id, vid_id, 'visible', img_id ))
                lwir = Image.open( self._imgpath % ( *frame_id[:-1], set_id, vid_id, 'lwir', img_id ) ).convert('L')
