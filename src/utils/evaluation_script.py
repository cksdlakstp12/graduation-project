...
class KAISTPedEval(COCOeval):
    ...
    def accumulate(self, p=None):
        ...
        def calculate_precision_recall_ap(self, ys, I0, npig, p, xx_graph):
            T, R, K, M = ys.shape
            precisions = -np.ones((T, R, K, M))  # 초기 precision 배열 준비
            aps = np.zeros((T, K))  # 각 IoU 임계값과 카테고리에 대한 AP 저장을 위한 배열

            for t in range(T):
                for k in range(K):
                    for m in range(M):
                        q = ys[t, :, k, m]  # 현재 threshold에서의 recall 값
                        p_list = []  # precision 값을 저장할 리스트
                        for r in range(R):
                            # precision 계산: TP / (TP + FP)
                            # TP는 recall에 npig를 곱한 값과 같고, FP는 FPPI에 I0를 곱한 값입니다.
                            tp = q[r] * npig
                            fp = xx_graph[t][r] * I0
                            if tp + fp > 0:
                                precision = tp / (tp + fp)
                        ...
                        # AP 계산을 위해 recall과 precision 배열을 준비
                        recall_sorted = np.sort(q)  # recall 값을 정렬
                        precision_sorted = p_list[np.argsort(q)]  # recall 값에 따라 precision 값을 정렬
                        
                        # 각 recall 레벨에서의 최대 precision 값을 계산
                        for i in range(precision_sorted.size - 2, -1, -1):
                            precision_sorted[i] = max(precision_sorted[i], precision_sorted[i + 1])
                        
                        # AP 계산: recall 변화량에 따른 precision의 합계
        ...
        for k, k0 in enumerate(k_list):
            ...
            for m, maxDet in enumerate(m_list):
                ...
                precision_sum = tp_sum / (tp_sum + fp_sum)  # Calculate Precision
                precision_sum[tp_sum + fp_sum == 0] = 0  # Avoid division by zero
                
                # Creating containers for mAP calculation
                aps = []

                # Process each threshold
                for t, (tp, fp, prec) in enumerate(zip(tp_sum, fp_sum, precision_sum)):
                    # Calculate recall and precision for current threshold
                    recall = tp / npig
                    precision = prec
                    
                    # Fixing recall and precision arrays for proper mAP calculation
                    recall_cal = np.concatenate(([0], recall, [1]))
                    precision_cal = np.concatenate(([0], precision, [0]))
                    
            # Fix precision by making sure it is non-increasing
            for i in range(len(precision_cal) - 1, 0, -1):
                precision_cal[i - 1] = max(precision_cal[i - 1], precision_cal[i])

            # Calculate Average Precision (AP) for the current class and threshold
            ap = np.sum((recall_cal[1:] - recall_cal[:-1]) * precision_cal[1:])
            aps.append(ap)
        ...
        # Calculate mean Average Precision (mAP) across all categories and thresholds
        #yy_graph.append(1 - recall)
        mAP = np.mean(aps)
        ...
        #print(f"precision_cal: {precision_cal}, recall_cal: {recall_cal}")
        print(f"mAP: {mAP}")
...
def evaluate(test_annotation_file: str, user_submission_file: str, phase_codename: str = 'Multispectral'):
    ...
    #my_reacall = eval_result['all'].eval['recall'][0][-1]
    #precision_all = eval_result['all'].eval['precision'][0][-1]
    AP_all = eval_result['all'].eval['mAP']

    #print(f"my_reacall: {my_reacall}")
