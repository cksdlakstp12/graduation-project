...
def main():
    ...
    if args.soft_teacher.student_checkpoint in "teacher" and \
        args.soft_teacher.teacher_checkpoint in "student":
    ...
    for epoch in range(0, epochs):
        ...
        if epoch >= 0:
            ...
            if MR_all < min_MR_all:
                min_MR_all = MR_all
                ckpt = f"{jobs_dir}/student_checkpoint_ssd300.pth.tar{str(epoch).zfill(3)}"
                cp_model = torch.load(ckpt)["model"]
                copy_student_to_teacher(t_model, cp_model)
                del cp_model
                torch.cuda.empty_cache()
                t_model.eval()
