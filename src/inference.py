...
def val_epoch_mAP(model: SSD300, dataloader: DataLoader, input_size: Tuple, min_score: float = 0.1) -> Dict:
    """Validate the model during an epoch
    ...
    device = next(model.parameters()).device
    results = dict()
    all_locs = torch.FloatTensor([]).to(device)
    all_labels = torch.FloatTensor([]).to(device)
    all_scores = torch.FloatTensor([]).to(device)
    all_boxes = list()
    all_labels = list()
    with torch.no_grad():
        for i, blob in enumerate(tqdm(dataloader, desc='Evaluating')):
            ...
            det_boxes_batch = torch.cat(det_boxes_batch, dim=0)
            det_labels_batch = torch.cat(det_labels_batch, dim=0)
            det_scores_batch = torch.cat(det_scores_batch, dim=0)

            all_locs = torch.cat([all_locs, det_boxes_batch], dim=0)
            all_labels = torch.cat([all_labels, det_labels_batch], dim=0)
            all_scores = torch.cat([all_scores, det_scores_batch], dim=0)
            all_boxes += boxes
            all_labels += labels
            ...
            for boxes_t, labels_t, scores_t, image_id in zip(det_boxes_batch, det_labels_batch, det_scores_batch, indices):
                ...
                results[image_id.item() + 1] = np.hstack([xywh_np, scores_np])
    mAP = calculate_mAP(all_locs, all_labels, all_scores, all_boxes, all_labels)
    print("mAP : ", mAP)
    return results
